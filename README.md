...
本次中期实验旨在原有基础记事本应用的基础上，扩展和增强应用功能，实现一个功能更加完善的个人笔记管理工具。通过本次实验，将学习Android应用开发中的数据存储、UI设计、菜单系统、内容提供者等核心技术的综合应用。主要实现了NoteList界面中笔记条目增加时间戳显示、添加笔记查询功能（根据标题内容或类型查询）、UI美化功能、笔记分类。

实验环境 开发平台：Android Studio
